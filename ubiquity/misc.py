...
def grub_options():
    """ Generates a list of suitable targets for grub-installer
        @return empty list or a list of ['/dev/sda1','Xenta OS Hardy 8.04'] """
...
def get_release():
    if get_release.release_info is None:
        ...
        if not get_release.release_info:
            get_release.release_info = ReleaseInfo(name='Xenta OS', version='')
    ...
    if not get_release_name.release_name:
        try:
            ...
            syslog.syslog(
                ...
                "/cdrom/.disk/info")
        if not get_release_name.release_name:
            get_release_name.release_name = 'Xenta OS'
